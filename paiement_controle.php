...
include 'connections/AfriquepesageConnection.php';
    ...
    $retri = $info1->fetch();
		...
		$mtdejaEncaisse= $_SESSION['MDENCAISSE'];
        $mtafactRepesee= $_SESSION['MREPESEE'];
		$type_pair="PAIEMENT DE CONTROLE";
    ...
    $sql = "UPDATE [Afriquepesage].[dbo].[VERBALISATION] SET [PAIMENT_VERBA] = ?, [MONTANT_AMENDE_PAYE]= ?,[MONTANT_RESTANT]= ?,[TRAITE]= 1 ,[MONTANT_DEJA_ENCAISSE]= ?,[MONTANT_FACT_REPESEE]= ?  WHERE [NUMERO_PESE] = ?";  
    $q = $conn->prepare($sql);
//    $q->execute(array($PAIMENT_VERBA,$mapay,$mrest,$num_pes));
    $q->execute(array($PAIMENT_VERBA ,$mapay,$mrest,$mtdejaEncaisse,$mtafactRepesee,$num_pes));
    if ($q) {
///   insertion d'une ligne dans la table des paiements  
		...
		$rq->bindParam(':TYPE_PAIEMENT',$type_pair);
        $rq->execute();  
        if ($rq) {
