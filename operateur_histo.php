...
//requette pour les pesees du wim
/** get weighing informations
                 * recuperer les infos de pesee
                 */
                $sth_P = $dbh->prepare('SELECT a."Numero_Pesee", a."Id_VP", a."Date_Pesee", a."Heure_Pesee", a."Unite_Mesure_Pesee", a."Vitesse_Moyenne_Pesee", a."Acceleration_Moyenne_Pesee", a."Selectionne_Pesee", a."Photo_Pesee", a."Commentaire_Pesee", a."Utilisateur_Pesee", a."poids_total_vehicule_Pesee", a."surcharge_Vehicule_Pesee", a."Vitesse_Min_Pesee", a."Vitesse_Max_Pesee", a."Erreur_Pesee", a."Type_Pesee", a."position_virgule", a.RDB$DB_KEY FROM "Pesee" a' );
                $sth_P->execute();
                $result_P = $sth_P->fetchAll();
foreach($result_P as $cle){
$numero_verb = "V".$cle['Numero_Pesee']. date('my') . $_SESSION['num_site'];
$counts = "SELECT  COUNT(*) OVER (PARTITION BY 1) as RowCnt FROM [Afriquepesage].[dbo].[VERBALISATION] where [ID_VERB] ='$numero_verb' AND [NOM_STE] = '$nom_site'";
        $verbs = $conn->query($counts);
        $total = $verbs->fetch();
		//if($total==0)
}
                        ...
                        <div class="col-sm-12">
                            ...
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    PESEE EN ATTENTE DE VERBALISATION
                                </div>
                                    ...
                                    <table class="table table-striped table-bordered table-hover table-full-width" id="sample_1" style="margin-top: 2%;">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>PHOTO</th>
                                                <th class="hidden-xs">POIDS VEHICULE</th>
                                                <th>POIDS SURCHARGE</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php
											...
											if($total==0){
                                                ...
                                                <tr class='clickable-row' >
                                                    <td><?php echo $value['Numero_Pesee'] ?></td>
                                                    <!--<td class="hidden-xs"><?php //echo $value['NUMERO_PESE'] ?></td>-->
                                                    <td><?php echo $value['Date_Pesee'] ?></td>
                                                    <td class="hidden-xs"><?php echo $value['Heure_Pesee'] ?></td>
                                                    <td class="hidden-xs"><?php echo $value['Utilisateur_Pesee'] ?></td>
                                                    <td><?php echo $value['Photo_Pesee'] ?></td>
                                                    <td class="hidden-xs"><?php echo $value['poids_total_vehicule_Pesee'] ?></td>
                                                    <td><?php echo $value['surcharge_Vehicule_Pesee'] ?></td>
                                                </tr>
                                                <?php
                                            }
											}
