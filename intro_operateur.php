...
//echo session_id()
    ...
    if (isset($_POST['search'])) {
        if (isset($_POST['produit_dangeureux'])) {
		 ...
		 if (isset($_POST['peseeC'])) {
            if ($_POST['peseeC'] != "") {
                $_SESSION['peseeC'] = $_POST['peseeC'];
            }
        }
...
//CHECK VERBALISATION NUMBER
        $count = "SELECT COUNT(*) OVER (PARTITION BY 1) as RowCnt FROM [Afriquepesage].[dbo].[VERBALISATION] where [ID_VERB] ='$verb' ";
...
//CHECK PREFERENTIAL CLIENT
    ...
    $teste = $dbh->query('SELECT  DISTINCT a."Numero_Pesee", a."poids_total_vehicule_Pesee" FROM "Pesee" a where a."Numero_Pesee" = ' . $_POST['num_pesee']);
    $teste->execute();
    $result2 = $teste->fetch();
    $poidsNC = $result2["poids_total_vehicule_Pesee"];
	
			if ($poidsNC <= 3500) {
            echo "<script>
			 
              alert(' ALERTE !!! POIDS DU CAMION NON CONFORME AU REGLEMENT 14');  
              window.location.href='intro_operateur.php';
        </script>";
			 ...
			 /** get vehicule informations
                 ...
                 */
                $sth_VP = $dbh->prepare('SELECT a."id_VP", a."nom_VP", a."poidsMax_VP", a."distFinMin_VP", a."distFinMax_VP", a."distFin_VP", a."longueurMax_VP", a."longueurMesure_VP", a."baseLongMesure_VP", a."hauteurMax_VP", a."hauteurMesure_VP", a."nbrMobiles_VP", a."image_VP", a."distancedebutMesuree_VP", a."distancedebutMin_VP", a."distancedebutMax_VP", a."erreur_VP", a.RDB$DB_KEY FROM "Vehicule_Pese" a where a."id_VP" =' . $_POST['num_pesee']);
                $sth_VP->execute();
                $result_VP = $sth_VP->fetch();
                $class_vehicule_uemoa = $result_VP['nom_VP'];
		/*récupérer le type de véhicule incon */
			 if ($result_VP['nom_VP'] == 'type inconnu')  {
                 $message = "Le type est inconnu";   
					echo "<script>
                alert('VERBALISATION IMPOSSIBLE : CLASSE DE TYPE INCONNU');
                window.location.href='intro_operateur.php';
                </script>";
				echo $message;
            ...
            ;
            //SET FOR DISPLAY THE TYPE AS INTERNATIONAL 
            $_SESSION['transport'] = "PREFERENTIEL";
            //THEN SET A MARKER THAT'LL SWITCH THE TRANSPORT TO PREFERENTIAL
            $_SESSION['preferentiel'] = 1;
            //FINALLY SINCE THE MARKER IS SET  INSERT TRANSPORT_AFFICHAGE IN DB
            $_SESSION['transport_affichage'] = "INTERNATIONAL";
...
//CHECK INFRACTION
            ...
            header("location: operateur_verbalisation_repesee.php");
            die;
        }
    }
...
//EXPORTATEUR REQUEST
            ...
            <div class="main-container">
                ...
                <div id = "responsive" class = "modal fade" tabindex = "-1" data-width = "300" style = "display: none;">
                    <div class = "modal-header">
                        ...
                        <h6 class = "modal-title mod"><center>INFORMATIONS DE VERBALISATION</center></h6>
                    </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <select  name="nationalite" class="form-control" required>
                                                    <?php
                                                    foreach ($countries_result as $key) {
                                                        if (rtrim($key['name']) == 'COTE DIVOIRE') {
                                                            ?>
                                                            <option value="<?php echo rtrim($key['name']); ?>" selected><?php echo rtrim($key['name']); ?></option>
                                                        <?php } else { ?>
	...
	<tr>
                                            <td valign="top">
                                                <label class="checkbox-inline">
                                                    <input type="checkbox" name="peseeC" class="flat-red" value="PESEE DE CONTROLE" >
                                                    Pesee de Controle
                                                </label>
                                            </td>
                                        </tr>
