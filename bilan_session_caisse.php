...
if(isset($_POST['search'])){
    ...
    $reqlistesession=$conn->prepare("SELECT * FROM [Afriquepesage].[dbo].[SESSION] WHERE [DATE_SESSION] ='".$_POST['selectdate']."' OR [DATE_FIN_SESSION] ='".$_POST['selectdate']."'");
...
if(isset($_POST['search2'])){
    if(isset($_POST['selectdate'])){
    $reqlistesession=$conn->prepare("SELECT * FROM [Afriquepesage].[dbo].[PAIEMENT] WHERE [DATE_PAIEMENT] ='".$_POST['selectdate']."'");
    $reqlistesession->execute();
    $sessionx=$reqlistesession->fetchAll();}
                        ...
                        <div class="tabbable">
                            ...
                            <div class="tab-content">
                                <div class="tab-pane in active" id="panel_tab2_example1">
                                    <p>
                                        <!-- DEBUT CONTENU-->
                                    <form method="POST" action="bilan_session_caisse.php">
                                    <table border="0">
                                        <tr >
                                        <th>CHOISIR LA DATE :
                                            <select id="seldate" name="selectdate" >
                                                <option value="NULL"></option>
                                         <?php foreach($redateresult as $lstdatesession)
                                                {
                                             ?>
                                                <option value="<?php echo $lstdatesession['DATE_SESSION'];?>"><?php echo date_format(date_create($lstdatesession['DATE_SESSION']),'d-m-Y');?> </option>
                                    ...
                                    <br/>
                                    <div id="imp">
                                        <strong>LISTES DES SESSIONS </strong>
                                    <table width="100%" border="1" class="table table-striped table-bordered table-hover table-full-width">
                                        ...
                                        ?>
                                        <tr>
                                            <td><?php echo  $session1['ID_SESSION']; ?> </td> 
                                            <td><?php echo  $session1['DATE_SESSION']." ".date_format(date_create($session1['DEBUT_SESSION']),'H:i:s'); ?> </td>                                             
                                            <td><?php echo  $session1['DATE_FIN_SESSION']." ".date_format(date_create($session1['FIN_SESSION']),'H:i:s'); ?> </td>
                                            <td><?php if($session1['STATUT_SESSION']==0){?> <span style="color: red">FERMER</span><?php } else { ?><span style="color: green">OUVERT</span><?php }?></td>
                                            <!--<td><?php //echo  date_format(date_create($session1['DEBUT_SESSION']),'H:i:s'); ?> </td>-->
                                            <?php 
                                            $reqrespo=$conn->prepare("SELECT * FROM [Afriquepesage].[dbo].[UTILISATEUR] WHERE [ID_USER]='".$session1['ID_USER']."'");
                                            $reqrespo->execute();
                                            ...
                                            $totencaiss=$conn->query("SELECT SUM([MONTANT_PAIEMENT]) as [TOTAL_PAIEMENT]FROM [Afriquepesage].[dbo].[PAIEMENT] where [ID_SESSION]='".$session1['ID_SESSION']."'"); 
                                            $chiffaff=$totencaiss->fetch();
                                    ?>
                                            <td><?php echo $chiffaff['TOTAL_PAIEMENT']; ?> </td>
                                            <td><a href="detailssession.php?idsess=<?php echo  $session1['ID_SESSION']; ?>&idut=<?php echo $session1['ID_USER']; ?>"><img src='assets/images/loupe.jpg' width="30" height="20" title="afficher"/> </a></td>
