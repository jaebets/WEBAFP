...
if (isset($_POST['search'])) {
    if (isset($_POST['dates'])) {
        ...
        $req = $conn->query("SELECT * FROM [Afriquepesage].[dbo].[VERBALISATION] where [DATE_VERB] ='$date_entry' AND [NOM_STE] = '$nom_site'");
        $req->execute();
        $result = $req->fetchAll();
    } else {
    ...
    if (isset($_POST['dates2']) && isset($_POST['dates3'])) {
        ...
        $req = $conn->query("SELECT * FROM [Afriquepesage].[dbo].[VERBALISATION] where [NOM_STE] = '$nom_site' AND [DATE_VERB] BETWEEN '$date_entry2' and '$date_entry3'");
        $req->execute();
        $result = $req->fetchAll();
    } else {
	...
	$id = $_SESSION['userid'];
    $dte = date("Ymd");
    $req = $conn->query("SELECT * FROM [Afriquepesage].[dbo].[VERBALISATION] WHERE [DATE_VERB] = '$dte' AND [NOM_STE]= '$nom_site'");
                            ...
                            <div class="panel panel-default">
                                ...
                                <div class="panel-body">
                                    <table class="table table-striped table-bordered table-hover table-full-width" id="sample_1" style="margin-top: 2%;">
                                        <thead>
                                            <tr>
                                                <th class="hidden-xs">DATE VERB</th>
                                                <th>Nº VERBALISATION</th>
                                                <th class="hidden-xs"> PROPRIETAIRE DU VEHICULE</th>
                                                <th>PROV/DEST</th>
                                        ...
                                        <tbody>
                                            ...
                                            foreach ($result as $value) {
                                                ...
                                                <tr class='clickable-row' data-href='verbalisation_recap.php?np=<?php echo $value['NUM_PESEE_WIM']; ?>&nv=<?php echo $value['ID_VERB']; ?>&dte=<?php echo $value['DATE_PESEE_WIM']; ?>' style="cursor: pointer;">
                                                    <td class="hidden-xs"><?php echo substr(($value['DATE_VERB']."-".$value['HEURE_VERBALISATION']),0,19) ?></td>
                                                    ...
                                                    <td class="hidden-xs"><?php if(preg_replace('/\s+/', '', $value['TRANSIT']) !="PREFERENTIEL") {echo preg_replace('/\s+/', '', $value['TRANSIT']);}else{echo "INTERNATIONAL";} ?></td>
