...
if ($q) {
    ...
    $log = "*" . date('d-m-Y H:i:s') . ": Verbalisation Nº " . $retri['ID_VERB'] . " Soldé chez *" . preg_replace('/\s+/', '', $_SESSION['nom']) . ' *' . preg_replace('/\s+/', '', $_SESSION['prenoms']) . "\n\n";
    fwrite($login_log, $log);
    fclose($login_log);
    echo "IMPRESSION TERMINEE";
    /*     * *************************************************************** */
///   mise à jour de la ligne correspondante dans la table des paiements  
    ...
    $rq->bindParam(':MONTANT_PAIEMENT', $solde);
    ...
    if ($rq) {
        /*         * *****************************JOURAL ENTRY****************** */
        $login_log = fopen("assets/log/caisses/journal" . date('d-m-Y'), "a+") or die("Unable to open file!");
        $log = "*" . date('d-m-Y H:i:s') . ": Paiement du solde de la Verbalisation Nº " . $retri['ID_VERB'] . " Soldé chez *" . preg_replace('/\s+/', '', $_SESSION['nom']) . ' *' . preg_replace('/\s+/', '', $_SESSION['prenoms']) . "\n\n";
        fwrite($login_log, $log);
        fclose($login_log);
        echo "IMPRESSION TERMINEE";
        /*         * *************************************************************** */
        ...
        if ($is) {
            /*             * *****************************JOURAL ENTRY****************** */
            $login_log = fopen("assets/log/caisses/journal" . date('d-m-Y'), "a+") or die("Unable to open file!");
            $log = "*" . date('d-m-Y H:i:s') . ": Reçu de la Verbalisation Nº " . $retri['ID_VERB'] . " généré par *" . preg_replace('/\s+/', '', $_SESSION['nom']) . ' *' . preg_replace('/\s+/', '', $_SESSION['prenoms']) . "\n\n";
            fwrite($login_log, $log);
            fclose($login_log);
            echo "IMPRESSION TERMINEE";
            /*             * *************************************************************** */
        ...
        $mv->bindParam(':ID_COMPTE_CLIENT', $idcpt);
        $mv->bindParam(':MONTANT_PAIEMENT', $solde);
