...
if ($mttapaye <= 0) {
    ...
    $log = "*" . date('H:i:s') . ": Erreur de Saisie d'un montant lors de l'enregistrement de la verbalisation Nº " . $verb . " par *" . preg_replace('/\s+/', '', $_SESSION['nom']) . ' *' . preg_replace('/\s+/', '', $_SESSION['prenoms']) . ' *' . $_SESSION['libelle'] . "\n\n";
    fwrite($login_log, $log);
    fclose($login_log);
    /*     * *************************************************************** */
    echo"<script>
                   alert('Vous avez saisi un montant inférieur ou égal à zéro, merci de corriger')
                   window.location.href = 'fiche_verbalisation.php';
                 </script>";
}
...
if ($mttapaye > $amdetot) {
    /*     * *****************************JOURAL ENTRY****************** */
    $login_log = fopen("assets/log/caisses/journal" . date('d-m-Y'), "a+") or die("Unable to open file!");
    $log = "*" . date('H:i:s') . ": Saisie d'un montant supérieur au montant de la verbalisation lors de l'enregistrement de la verbalisation Nº " . $verb . " par *" . preg_replace('/\s+/', '', $_SESSION['nom']) . ' *' . preg_replace('/\s+/', '', $_SESSION['prenoms']) . ' *' . $_SESSION['libelle'] . "\n\n";
    fwrite($login_log, $log);
    fclose($login_log);
    /*     * *************************************************************** */
    echo"<script>
...
//        echo $modregl ;
   ...
   // echo " B " . $reqest['ID_COMPTE_CLIENT'];
    ...
    if (empty($reqest['ID_COMPTE_CLIENT'])) {
        ...
        $log = "*" . date('H:i:s') . ": Erreur de Saisie d'un compte client pour debit lors de l'enregistrement de la verbalisation Nº " . $verb . " par *" . preg_replace('/\s+/', '', $_SESSION['nom']) . ' *' . preg_replace('/\s+/', '', $_SESSION['prenoms']) . ' *' . $_SESSION['libelle'] . "\n\n";
        fwrite($login_log, $log);
        fclose($login_log);
        /*         * *************************************************************** */
        echo" <script>
        ...
        if ($sl) {
            ...
            if ($debit >= 0) {
                ...
                /*                 * *****************************JOURAL ENTRY****************** */
                $login_log = fopen("assets/log/compte_clients/journal" . date('d-m-Y'), "a+") or die("Unable to open file!");
                $log = "*" . date('H:i:s') . ": Le Compte *" . $_POST['MODE_REGLEMENT'] . " a été debité du montant = " . $mttapaye . " lors de l'enregistrement de la verbalisation Nº " . $verb . " par *" . preg_replace('/\s+/', '', $_SESSION['nom']) . ' *' . preg_replace('/\s+/', '', $_SESSION['prenoms']) . ' *' . $_SESSION['libelle'] . "\n\n";
                fwrite($login_log, $log);
                fclose($login_log);
                /*                 * *************************************************************** */
                ...
                /*                 * *****************************JOURAL ENTRY****************** */
                $login_log = fopen("assets/log/compte_clients/journal" . date('d-m-Y'), "a+") or die("Unable to open file!");
                $log = "*" . date('H:i:s') . ": Le Compte *" . $_POST['MODE_REGLEMENT'] . " n'a pu etre débité (insuffisance de fond) lors de l'enregistrement de la verbalisation Nº " . $verb . " par *" . preg_replace('/\s+/', '', $_SESSION['nom']) . ' *' . preg_replace('/\s+/', '', $_SESSION['prenoms']) . ' *' . $_SESSION['libelle'] . "\n\n";
                fwrite($login_log, $log);
                fclose($login_log);
                /*                 * *************************************************************** */
                echo"<script>
...
?>
                    ...
                    <table width="100%" border="0" style="margin-top:5%"> 
                        <tr>
                            ...
                            <td><div align="center"><strong>Imprim&eacute; le : <?php echo date("d/m/Y") ?></strong></div></td>
                        </tr>
                    </table>
...
<?php ?>
    ...
    <div class="modal-footer">
        ...
        <a class="btn btn-red " href="fiche_verbalisation.php">
            ANNULER
        </a>
    </div>
</div>  
